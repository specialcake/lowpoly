...
class Renderer: NSObject {
    ...
    init(device: MTLDevice, modelMatrix: float4x4, forResourse name: String, withExtension ext: String) {
        ...
        renderPipeline = Renderer.buildPipeline(device: device, vertexDescriptor: vertexDescriptor)
    ...
    func updateScene(drawable: CAMetalDrawable) {
        ...
        let aspectRatio = Float(drawable.texture.width / drawable.texture.height)
        projectionMatrix = float4x4(perspectiveProjectionFov: radian(45), aspectRatio: aspectRatio, nearZ: 0, farZ: 100)
    ...
    func draw(drawable: CAMetalDrawable, pipeline: MTLRenderPipelineState) {
